...
- **R-FI-1 — Non-registered below threshold** — Non-registered entities do not file ALV returns. (Trigger: turnover below EUR 15,000 (from 2025), not registered.)
- **R-FI-2 — Partial exemption** — Partial exemption under AVL Section 117 requires reviewer. (Trigger: mixed supplies.)  _(AVL Section 117)_
- **R-FI-3 — Construction reverse charge (AVL Section 8c)** — Construction reverse charge requires specialist classification. (Trigger: construction services.)  _(AVL Section 8c)_
- **R-FI-4 — Aland Islands goods trade** — Aland is outside EU VAT territory for goods. Special import/export treatment required. (Trigger: goods imported/exported to Aland.)
- **R-FI-5 — Special schemes** — Margin/travel agent schemes out of scope.
...
- **Standard rate default** — Default. Sales: Code 301/305. Input: Code 307.  _(AVL Section 84)_

### 5.2 Reduced 14%  (note: reduced to 13.5% from Jan 2026) (AVL Section 85)

- **Reduced rate 14%** — Food, restaurant/catering, animal feed.  _(AVL Section 85)_

### 5.3 Reduced 10% (AVL Section 85a)

- **Reduced rate 10%** — Books, newspapers, medicines, passenger transport, accommodation, cultural/sporting events.  _(AVL Section 85a)_

### 5.4 Zero rate / exports

- **Zero rate / exports** — Exports: Code 314. Intra-EU goods: Code 311. Intra-EU services: Code 312.

### 5.5 Exempt without credit (AVL Sections 34-60)

- **Exempt without credit** — Financial, insurance, healthcare, education, postal, gambling, residential rental.  _(AVL Sections 34-60)_

### 5.6 Reverse charge — EU services (AVL Section 9/65)

- **Reverse charge EU services** — Base: Code 313. Input: Code 307.  _(AVL Section 9/65)_

### 5.7 Reverse charge — EU goods (AVL Section 26a)

- **Reverse charge EU goods** — Base: Code 309. Input: Code 307.  _(AVL Section 26a)_

### 5.8 Reverse charge — non-EU services

- **Reverse charge non-EU services** — Base: Code 319. Input: Code 307.

### 5.9 Construction reverse charge (AVL Section 8c)

- **Construction reverse charge** — Base: Code 320. Flag R-FI-3 for reviewer.  _(AVL Section 8c)_

### 5.10 Single input code

- **Single input code** — Finland aggregates ALL deductible input ALV into a single Code 307. No separate input codes by rate or source.

### 5.11 Representation 50% rule

- **Representation 50% rule** — Entertainment/representation: 50% of input ALV deductible (AVL Section 114). Requires documented business purpose.  _(AVL Section 114)_

### 5.12 Aland Islands (AVL Section 68a)

- **Aland Islands treatment** — Goods to/from Aland treated as import/export. Services follow normal place-of-supply rules.  _(AVL Section 68a)_
...
- **Entertainment** — Default: block. Question: "Representation (50% deductible) or personal?"
...
- **Individual incoming** — Default: 25.5%.

### 6.6 Foreign incoming — *Default:* 25.5%.

- **Foreign incoming** — Default: 25.5%.
...
- **Aland trade** — Default: flag reviewer.
...
- **Entity type inference** — Oy = company; toiminimi = sole trader.

### 9.2 ALV registration — *Fallback:* "Are you ALV-registered?"

- **ALV registration fallback** — Fallback: "Are you ALV-registered?"

### 9.3 Y-tunnus — *Fallback:* "Y-tunnus? (NNNNNNN-N)"

- **Y-tunnus fallback** — Fallback: "Y-tunnus? (NNNNNNN-N)"
...
- **Construction fallback** — Fallback: "Do you provide construction services?"

### 9.7 Aland — *Fallback:* "Do you trade with Aland Islands?"

- **Aland fallback** — Fallback: "Do you trade with Aland Islands?"
