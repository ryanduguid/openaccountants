...
- **Prepayment computation formula** — prior_year_ESt = income tax from last assessment minus_wage_tax = Lohnsteuer withheld minus_KapESt = Kapitalertragsteuer withheld minus_credits = other credits (foreign tax credits etc.) prepayment_base = prior_year_ESt - wage_tax - KapESt - credits annual_prepayment = prepayment_base (rounded down to nearest EUR divisible by 4) quarterly_instalment = annual_prepayment / 4  _(EStG Paragraph 37)_
...
- **Weekend/holiday shift rule** — If the 10th falls on a Saturday, Sunday, or public holiday, the due date shifts to the next business day  _(AO Paragraph 108 Abs. 3)_

### 5.4 Adjustment window

- **Adjustment window** — The Finanzamt may adjust prepayments until the 15th month after the end of the calendar year. For tax year 2025: adjustments possible until 31 March 2027.

### 5.5 Year-end settlement

- **Year-end settlement formula** — final_tax = ESt on actual income total_prepaid = sum of all Vorauszahlungen if total_prepaid > final_tax: Erstattung (refund) if total_prepaid < final_tax: Nachzahlung (balance due)
...
**Late payment surcharge elements**  _(AO Paragraph 240 Abs. 3)_
...
- **Late payment surcharge computation** — if payment_date > due_date + 3_days: months_late = number of commenced months from due_date surcharge = floor(unpaid_amount / 50) * 50 * 1% * months_late

### 6.3 Interest on arrears (Nachzahlungszinsen)

- **Interest on arrears rate** — 0.15% per month (1.8% per year)  _(AO Paragraph 233a)_
- **Interest-free period** — Interest-free period: 15 months after end of tax year. Applies to both underpayments and overpayments.  _(AO Paragraph 233a)_
