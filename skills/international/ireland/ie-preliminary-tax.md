...
- **Minimum viable inputs** — prior year final tax liability (income tax + USC + PRSI) for the 100% rule, OR current year estimated income for the 90% rule.
- **Recommended inputs** — filing method (paper vs ROS), PAYE credits from employment, CGT obligations (separate), current year income expectations.
- **Ideal inputs** — complete prior year Form 11, Revenue assessment, current year P&L projection, PAYE certificates.
...
- **Direct debit identification** — If the client pays preliminary tax by direct debit instalments (105% of pre-prior-year rule), monthly debits to Revenue throughout the year are direct debit preliminary tax payments. Flag for reviewer.
...
- **Safe harbour compliance** — Taxpayer meets obligation if they pay at least ONE of these amounts.

### 5.2 What constitutes "tax liability"

- **Definition of tax liability** — Preliminary tax = income tax + USC + PRSI (Class S at 4.125%). PRSI minimum = EUR 650 if income > EUR 5,000.

### 5.3 Computation steps

- **Computation steps** — prior_year_liability = income_tax + USC + PRSI option_A = prior_year_liability x 100% option_B = estimated_current_year_liability x 90% preliminary_tax = min(option_A, option_B) (or simply pay option_A for safety) net_due = preliminary_tax - PAYE_credits - withholding_tax

### 5.4 105% of pre-prior-year (direct debit)

- **105% direct debit rule** — For clients on direct debit: 105% of the pre-prior-year liability, spread in equal monthly instalments. This is a third safe harbour.

### 5.5 First year rule

- **First year rule** — First year of self-assessment: 100% rule is unavailable (no prior year). Must use 90% of estimated current year.
...
- **Surcharge applies to return not payment** — The surcharge applies to the RETURN, not the payment. Even if payment is on time, a late return triggers the surcharge.
...
- **PRSI collection method** — PRSI is included in the preliminary tax payment and collected through self-assessment.
