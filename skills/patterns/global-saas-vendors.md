...
**Cloud and hosting vendor lookup table**
...
**Developer tools vendor lookup table**
...
**Design tools vendor lookup table**
...
**Communication vendor lookup table**
...
**Project management vendor lookup table**
...
**Accounting and finance vendor lookup table**
...
**Legal and compliance vendor lookup table**
...
**Other SaaS vendor lookup table**
...
- **Assuming all SaaS is deductible** — A subscription is only a business expense if it is used for business. Spotify, Netflix, Disney+, and similar are NOT business expenses unless the client can demonstrate genuine business use (e.g., a cafe playing background music via a business Spotify account).
- **Missing reverse charge obligations** — When an EU-based business purchases from a non-EU SaaS vendor (or a vendor in a different EU member state), the buyer typically must self-assess VAT via the reverse charge mechanism. Failing to do so creates a VAT compliance gap even though it is usually net-neutral (output VAT = input VAT if fully taxable).
- **Confusing payment processor charges with SaaS subscriptions** — Stripe, PayPal, and Wise appear in this table for their subscription/platform fees, but their transaction-level entries (payouts, transfers) are covered in `global-payment-processors.md`. Do not double-classify.
- **Ignoring billing entity changes** — Many vendors restructure their billing entities (e.g., GitHub moving under Microsoft Ireland, Heroku under Salesforce). Always check the actual invoice to determine the billing entity for VAT purposes.
- **Annual vs. monthly billing** — Large annual charges should be accrued across the period they cover, not expensed in the month of payment, if the client uses accrual accounting.
