...
- **R-IS-1 — Non-registered** — Non-registered entities do not file VSK returns. (Trigger: not registered, turnover below ISK 2,000,000.)
- **R-IS-2 — Partial exemption** — Mixed supplies require proportional attribution. Use a qualified tax adviser.
- **R-IS-3 — Fishing vessel / special investment schemes** — Fishing vessel and special investment zone schemes out of scope.
- **R-IS-4 — Faroe Islands trade** — Faroe Islands are a separate Danish territory, not part of Iceland for VSK. Specialist review needed. (Trigger: trade with Faroe Islands.)
...
- **Standard 24%** — Default. Sales: Field A/C. Input: Field J.  _(Act 50/1988, Art. 14(1))_

### 5.2 Reduced 11% (Art. 14(2))

- **Reduced 11%** — Food, accommodation, books, newspapers, geothermal hot water, radio/TV licence.  _(Art. 14(2))_

### 5.3 Zero rate (Art. 12, 42)

- **Zero rate** — Exports: Field E. International transport: Field E.  _(Art. 12, 42)_

### 5.4 Exempt

- **Exempt categories** — Financial, insurance, healthcare, education, postal, gambling, residential rental.

### 5.5 Reverse charge — ALL foreign services (Art. 35)

- **Reverse charge all foreign services** — No EU/non-EU distinction. All services received from abroad: self-assess at 24%. Field G (base), Field H (output). Input in Field L.  _(Art. 35)_

### 5.6 Imports of goods

- **Imports of goods** — Import VSK assessed by customs. Input: Field K.

### 5.7 No intra-community rules

- **No intra-community rules** — Iceland has NO intra-EU acquisition/supply mechanism. Goods from EU countries are imports. Services from anywhere are reverse charge.
...
- **Vehicle costs default/question** — Default: 0%. Question: "Exclusive business use?"
...
- **Foreign SaaS default** — Default: reverse charge at 24%.
...
- **Tourism/hospitality default/question** — Default: 11%. Question: "Confirm accommodation vs food service."

### 6.6 Import goods — *Default:* check customs declaration.

- **Import goods default** — Default: check customs declaration.
...
- **Icelandic language glossary** — leiga (rent), laun (salary), vextir (interest), millifærsla (transfer), reiðufe (cash)
...
- **Entity type inference** — ehf. = company; einstaklingsrekstur = sole trader.

### 9.2 VSK registration — *Fallback:* "VSK-registered?"

- **VSK registration fallback** — Fallback: "VSK-registered?"

### 9.3 Kennitala — *Fallback:* "Kennitala? (10 digits)"

- **Kennitala fallback** — Fallback: "Kennitala? (10 digits)"

### 9.4 VSK number — *Fallback:* "VSK number? (IS + 5-6 digits)"

- **VSK number fallback** — Fallback: "VSK number? (IS + 5-6 digits)"

### 9.5 Filing period — Bi-monthly default.

- **Filing period default** — Bi-monthly default.

### 9.6 Tourism/hospitality — *Fallback:* "Tourism or accommodation business?"

- **Tourism/hospitality fallback** — Fallback: "Tourism or accommodation business?"

### 9.7 Exports — *Fallback:* "Do you export goods or services?"

- **Exports fallback** — Fallback: "Do you export goods or services?"

## Section 10 — Reference material

### Sources

- **Source 1** — Log um virðisaukaskatt (Act No. 50/1988, as amended)
- **Source 2** — Regulation No. 630/2008
- **Source 3** — EEA Agreement (cross-border service rules)
- **Source 4** — Central Bank of Iceland exchange rates
